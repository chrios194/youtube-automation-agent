...
require('dotenv').config({ path: process.env.WORKSPACE_ENV_FILE || path.join(__dirname, '.env') });
...
const { getWorkspaceRuntime } = require('./utils/workspace-runtime');
...
class YouTubeAutomationAgent {
  constructor() {
    ...
    this.runtime = getWorkspaceRuntime();
    this.db = null;
  ...
  async initialize() {
    try {
      ...
      if (this.runtime.enabled) {
        this.logger.info(`Starting isolated workspace account: ${this.runtime.accountId}`);
      }

      // Initialize database
      this.logger.info('Initializing database...');
      this.db = new Database({ runtime: this.runtime });
      ...
      this.credentials = new CredentialManager({ runtime: this.runtime });
  ...
  setupAPI() {
    ...
    this.app.get('/health', (req, res) => {
      res.json({
        ...
        workspace: this.runtime.enabled ? {
          id: this.runtime.workspaceId,
          accountId: this.runtime.accountId,
          accountName: this.runtime.accountName,
          strictMode: this.runtime.strictMode
        } : null
      });
    });
  ...
  async start() {
    ...
    const HOST = this.runtime.host || '127.0.0.1';
    this.app.listen(PORT, HOST, () => {
      console.log(chalk.green(`\n✅ YouTube Automation Agent running on ${HOST}:${PORT}`));
      console.log(chalk.gray('─'.repeat(50)));
      console.log(chalk.white('📊 Dashboard: ') + chalk.cyan(`http://${HOST}:${PORT}`));
      console.log(chalk.white('🔧 API Health: ') + chalk.cyan(`http://${HOST}:${PORT}/health`));
      console.log(chalk.white('📅 Schedule: ') + chalk.cyan(`http://${HOST}:${PORT}/schedule`));
      console.log(chalk.white('📈 Analytics: ') + chalk.cyan(`http://${HOST}:${PORT}/analytics`));
      console.log(chalk.gray('─'.repeat(50)));
      if (this.setupRequired) {

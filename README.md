...
## v3.0.0 — World architecture

AgentTube v3 adds the **federated portfolio control plane**: bounded creator workspace pods remain isolated while a portfolio manifest coordinates governance, objectives, health, and scale without pooling raw creator credentials or customer data. Strict workspace mode now requires encrypted local secret storage through `AGENTTUBE_MASTER_KEY`.

Start with [`docs/WORLD_RELEASE.md`](docs/WORLD_RELEASE.md), copy `config/portfolio.example.json` to `config/portfolio.json`, and run `npm run portfolio:validate`. Creator identities are governed through the Creator Identity & Business Passport contract in `config/passports/creator-passport.example.json`.
...
> **Operating a creator portfolio?** Do not use the classic one-channel setup as your production topology. Configure an isolated account workspace with [`docs/multi-account-workspaces.md`](docs/multi-account-workspaces.md), then use `node workspace.js credentials --account <account-id>` for each account before starting the workspace.
...
## Run multiple isolated creator accounts in parallel

For a bounded creator pod, run one isolated AgentTube process per creator account rather than placing multiple channel credentials and audiences inside the same database and publish queue. Each process receives its own port, SQLite state, credentials, tokens, content history, scheduler, and approval key.

```bash
cp config/workspace.example.json config/workspace.json
# Create each account environment file referenced by the manifest.
npm run workspace:validate
npm run workspace:status
npm run workspace:start
```

The workspace launcher enables strict account mode and requires an `API_KEY` in every account environment file. It intentionally supports up to 12 account processes per workspace; run multiple independent workspaces as the portfolio grows. Read [`docs/multi-account-workspaces.md`](docs/multi-account-workspaces.md) for the account-isolation model, manifest contract, deployment boundaries, and incident guidance.

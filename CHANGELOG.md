...
### Added
- Federated portfolio control plane (`portfolio.js`) for coordinating bounded creator workspace pods without merging secrets or creator databases.
- Creator Identity & Business Passport example governing synthetic identity disclosure, editorial point of view, recurring shows, offers, and platform permissions.
- AES-256-GCM encrypted credential/token files in workspace strict mode using `AGENTTUBE_MASTER_KEY`.
- Platform adapter and commerce-boundary contracts so new distribution/monetization systems fail closed instead of pretending unsupported integrations exist.
- Integrity-checked account-runtime backup and verification utilities.
- World-release architecture and global operations runbook.

### Changed
- Workspace validation now requires a 32+ character master key for encrypted secrets.
- Package identity promoted to AgentTube 3.0.0, a federated creator-business operating system.
